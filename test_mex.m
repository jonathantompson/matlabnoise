...
title('Hermite2DDeriv');

err = abs(noise_deriv - noise);
assert(max(err(:)) < 1e-6, ...
  'scalar field from Hermite2DDeriv does not match Hermite2D!');

% Check the gradient using finite differences (central)
epsilon = 1e-6;
[pos, ~] = Hermite2DDeriv(X + epsilon, Y);
[neg, ~] = Hermite2DDeriv(X - epsilon, Y);
dx = (pos - neg) / (2 * epsilon);
[pos, ~] = Hermite2DDeriv(X, Y + epsilon);
[neg, ~] = Hermite2DDeriv(X, Y - epsilon);
...
title('Hermite3D');
% 3D Gradient
[noise_deriv, gradNoise] = Hermite3DDeriv(X, Y, Z);
err = abs(noise_deriv - noise);
assert(max(err(:)) < 1e-6, ...
  'scalar field from Hermite3DDeriv does not match Hermite3D!');

figure;
plot_3D_scalar_and_grad(noise_deriv, gradNoise);
title('Hermite3DDeriv');
set(gcf,'Renderer','OpenGL')
title('Hermite3DDeriv');

% Check the gradient using finite differences (central)
epsilon = 1e-6;
[pos, ~] = Hermite3DDeriv(X + epsilon, Y, Z);
[neg, ~] = Hermite3DDeriv(X - epsilon, Y, Z);
dx = (pos - neg) / (2 * epsilon);
[pos, ~] = Hermite3DDeriv(X, Y + epsilon, Z);
[neg, ~] = Hermite3DDeriv(X, Y - epsilon, Z);
dy = (pos - neg) / (2 * epsilon);
[pos, ~] = Hermite3DDeriv(X, Y, Z + epsilon);
[neg, ~] = Hermite3DDeriv(X, Y, Z - epsilon);
